    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-green-900 to-slate-800 flex items-center justify-center p-6">
      <div className="max-w-md w-full">
        <div className="bg-black/30 border border-green-800/30 rounded-2xl p-8">
          ...
          <input type="text" placeholder="请输入兑换码" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} className="w-full p-4 bg-black/30 border border-green-700/30 rounded-xl text-white placeholder-green-600/50 focus:outline-none focus:border-green-500 mb-4 text-center text-lg font-mono" />

          <button onClick={redeem} disabled={loading || !session} className="w-full py-4 bg-gradient-to-r from-green-600 to-emerald-700 text-white rounded-xl font-medium disabled:opacity-50">
            {loading ? '兑换中...' : '立即兑换'}
          </button>
          ...
          <button onClick={() => router.push('/')} className="w-full mt-4 py-3 text-green-500 hover:text-green-400">返回首页</button>
        </div>
      </div>
    </div>
  )
