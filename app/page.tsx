  ...
  if (session) {
    return (
      <div className={`min-h-screen ${theme.bg}`}>
        <header className={`border-b ${theme.border} bg-${darkMode ? 'gray-900' : 'white'}`}>
          <div className="max-w-3xl mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              <div className={`px-4 py-1.5 rounded-full ${darkMode ? 'bg-gray-800' : 'bg-gray-100'} border ${theme.border}`}>
                <span className={`text-sm font-medium ${theme.textSecondary}`}>💰 {isTestUser ? '无限' : credits} 积分</span>
              </div>
              <button onClick={() => setShowSaved(!showSaved)} className={`px-4 py-1.5 rounded-full text-sm ${showSaved ? theme.button : (darkMode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900')}`}>
                {showSaved ? '📋 历史' : `⭐ ${savedList.length}`}
              </button>
              <button onClick={toggleDarkMode} className={`p-2 rounded-full ${darkMode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900'}`}>
                {darkMode ? '☀️' : '🌙'}
              </button>
              <div className={`w-8 h-8 rounded-full bg-gray-900 dark:bg-gray-600 flex items-center justify-center text-white text-sm`}>
        ...
        <main className="max-w-3xl mx-auto px-6 py-8">
          <div className={`${theme.card} rounded-2xl p-6 mb-6 shadow-sm`}>
              ...
              <div className={`mt-6 p-4 ${darkMode ? 'bg-gray-800' : 'bg-gray-50'} rounded-xl`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => saveAudio({id: Date.now().toString(), text, url: audioUrl, time: new Date().toISOString()})} className={`text-sm ${theme.accent}`}>⭐ 保存</button>
                    <a href={audioUrl} download="audio.mp3" className={`text-sm ${theme.accent}`}>下载</a>
                  </div>
                </div>
                ...
                {displayList.map((item) => (
                  <div key={item.id} className={`flex items-center gap-4 p-4 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
                    ...
                    <div className="flex gap-2">
                      {showSaved && (
                        <button onClick={() => deleteSaved(item.id)} className={`p-2 text-gray-400`}>🗑️</button>
                      )}
                      {!item.saved && !showSaved && (
                        <button onClick={() => saveAudio(item)} className={`p-2 ${theme.accent}`}>⭐</button>
                      )}
  ...
  return (
    <div className={`min-h-screen ${theme.bg} flex items-center justify-center p-6`}>
      <div className="max-w-sm w-full">
        ...
        <div className={`${theme.card} rounded-2xl p-6 shadow-sm`}>
          <button onClick={() => signIn()} className={`w-full py-3 ${theme.button} rounded-xl text-sm font-medium`}>
            立即体验
          </button>
          <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div><div className="text-lg mb-1">🎙️</div><p className={`${theme.textSecondary} text-xs`}>川哥音色</p></div>
              <div><div className="text-lg mb-1">⚡</div><p className={`${theme.textSecondary} text-xs`}>极速生成</p></div>
              <div><div className="text-lg mb-1">💰</div><p className={`${theme.textSecondary} text-xs`}>1积分=1万字</p></div>
            </div>
          </div>
        </div>
      </div>
